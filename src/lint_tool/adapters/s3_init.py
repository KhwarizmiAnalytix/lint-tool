...
def packaged_s3_config() -> dict:
    """Load the binary map shipped with this package."""
    try:
        from importlib.resources import files
    except ImportError:  # pragma: no cover
        from importlib_resources import files  # type: ignore[no-redef]

    data = files("lint_tool.adapters").joinpath("s3_init_config.json").read_text(
        encoding="utf-8"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to config json that describes where to find binaries and "
            "hashes. Defaults to the map shipped with lint-tool."
        ),
    )
    parser.add_argument(
        "--linter",
